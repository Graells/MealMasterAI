...
describe("NewDietForm", () => {
  test("should submit the form when all fields are valid", () => {
    ...
    const { getByLabelText, getByText, click, toHaveBeenCalledTimes } = render(
      <CreateMealForm onMealSubmit={handleSubmit} />
    );

    fireEvent.change(getByLabelText("Title"), {
      target: { value: "My new diet" },
    });
    ...
    fireEvent.change(getByLabelText("Weight (in kg):"), {
      target: { value: "70" },
    });
    fireEvent.change(getByLabelText("Height (in cm):"), {
      target: { value: "170" },
    });
    fireEvent.change(getByLabelText("Activity Level:"), {
      target: { value: "MODERATE" },
    });
    fireEvent.change(getByLabelText("Dietary Preferences:"), {
      target: { value: "Vegetarian" },
    });
    fireEvent.change(getByLabelText("Weight Goal:"), {
      target: { value: "LOSE" },
    });
    fireEvent.change(getByLabelText("Amount(in kg):"), {
      target: { value: "5" },
    });
    fireEvent.change(getByLabelText("Time Frame (in weeks):"), {
      target: { value: "10" },
    });
    fireEvent.change(getByLabelText("Eating Frequency (meals per day):"), {
      target: { value: "3" },
    });
